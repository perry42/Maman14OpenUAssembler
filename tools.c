/* Elizur Leiman , Yarden Danieli */

/************************************************************** tools.c **************************************************************/
/*
    The tools.c file contains all functions  that are used as tools to proces the different lables, detect and characterize
    them, and also process their encoding. Functions here also used to handle errors output messages.
*/
/**************************************************************************************************************************************/

#include <stdio.h>
#include <string.h>
#include <ctype.h>
#include <stdlib.h>
#include <math.h>

#include "prototypes.h"
#include "assembler.h"
#include "externals.h"
#include "tools.h"

/* This function extracts bits, given start and end positions of the bit-sequence (0 is LSB) */
unsigned int extract_bits(unsigned int word, int start, int end)
{
    unsigned int result;
    int length = end - start + 1;                /* Length of bit-sequence */
    unsigned int mask = (int)pow(2, length) - 1; /* Creating a '111...1' mask with above line's length */

    mask <<= start;                             /* Moving mask to place of extraction */
    result = word & mask;                       /* The bits are now in their original position, and the rest is 0's */
    result >>= start;                           /* Moving the sequence to LSB */
    return result;
}

/* Converting a word to 2 digits in base 64 (as a string) */
char *convert_to_base_64(unsigned int num)
{
    char *base64_seq = (char *)malloc(BASE64_SEQUENCE_LENGTH);

    /* To convert from binary to base 64 we can just take the 6 right binary digits and 6 left */
    base64_seq[0] = base64[extract_bits(num, 6, 11)];
    base64_seq[1] = base64[extract_bits(num, 0, 5)];
    base64_seq[2] = '\0';

    return base64_seq;
}

/* This function checks if a string is a number (all digits) */
boolean is_number(char *seq)
{
    if (end_of_line(seq))
        return FALSE;
    if (*seq == '+' || *seq == '-') /* A number can contain a plus or minus sign */
    {
        seq++;
        if (!isdigit(*seq++))
            return FALSE; /* But not only a sign */
    }

    /* Check that the rest of the token is made of digits */
    while (!end_of_line(seq))
    {
        if (!isdigit(*seq++))
            return FALSE;
    }
    return TRUE;
}

/* This function checks if a given sequence is a valid string (with quotation marks "") */
boolean is_string(char *string)
{
    if (string == NULL)
        return FALSE;

    if (*string == '"')             /* Starts with " */
        string++;
    else
        return FALSE;

    while (*string && *string != '"')   /* Ends with " */
    { /* Goes until end of string */
        string++;
    }

    if (*string != '"') /* A string must end with " */
        return FALSE;

    string++;
    if (*string != '\0') /* String token must end after the ending " */
        return FALSE;

    return TRUE;
}

/* This function inserts given ARE two bits into given informational bit-sequence (the info argument is being shifted left) */
unsigned int put_are(unsigned int info, int are)
{
    return (info << ARE_BITS) | are; /* | bitwise operand allows insertion of the two bits because 1 + 0 = 1 */
}

/* This function creates a file name by appending suitable extension (by type) to the original string */
char *create_file_name(char *original, int type)
{
    char *modified = (char *)malloc(strlen(original) + MAX_EXTENSION_LENGTH);
    if (modified == NULL)
    {
        fprintf(stderr, "Dynamic allocation error.");
        exit(ERROR);
    }

    strcpy(modified, original); /* Copying original filename to the bigger string */

    /* Concatenating the required file extension */

    switch (type)
    {
    case FILE_MACRO:
        strcat(modified, ".am");
        break;

    case FILE_INPUT:
        strcat(modified, ".as");
        break;

    case FILE_OBJECT:
        strcat(modified, ".ob");
        break;

    case FILE_ENTRY:
        strcat(modified, ".ent");
        break;

    case FILE_EXTERN:
        strcat(modified, ".ext");
    }
    return modified;
}

/* This function inserts a given word to instructions memory */
void encode_to_instructions(unsigned int word)
{
    instructions[ic++] = word;
}

/* This functions returns 1 if there's an error (e.g global variable err has changed) */
int is_error()
{
    return err != NO_ERROR;
}

/* This function receives line number as a parameter and prints a detailed error message
   accordingly to the error global variable */
void write_error(int line_num)
{
    if (line_num > 0)
    {
        fprintf(stderr, "Error (line %d): ", line_num);
    }
    switch (err)
    {
    case SYNTAX_ERR:
        fprintf(stderr, "A first non-blank character must be a letter or a dot.\n");

        break;

    case MACRO_ALREADY_EXIST:
        fprintf(stderr, "Defined macro is already exists.\n");

        break;

    case MACRO_CANNOT_BE_COMMAND:
        fprintf(stderr, "Macro has a name of an assembly command.\n");

        break;

    case MACRO_CANNOT_BE_REGISTER:
        fprintf(stderr, "Macro has the same name as a hardware register.\n");

        break;

    case MACRO_TO_MANY_OR_MISSING_PARAM:
        fprintf(stderr, "The number of arguments in macro declaration , after \"mcro\", \"endmcro\"  or macro name is invalid.\n");

        break;
    case LABEL_ALREADY_EXISTS:
        fprintf(stderr, "Call for an existing lable.\n");

        break;

    case LABEL_TOO_LONG:
        fprintf(stderr, "Lable's name is out of range. (LABEL_MAX_LENGTH: %d).\n", LABEL_LENGTH);

        break;

    case LABEL_INVALID_FIRST_CHAR:
        fprintf(stderr, "Label's name starts with an invalid character.\n");

        break;

    case LABEL_ONLY_ALPHANUMERIC:
        fprintf(stderr, "Label's name contains invalid characters.\n");

        break;

    case LABEL_CANT_BE_COMMAND:
        fprintf(stderr, "Label has the same name as that of an assembly command.\n");

        break;

    case LABEL_CANT_BE_REGISTER:
        fprintf(stderr, "Label has the same name as that of a hardware register.\n");

        break;

    case LABEL_ONLY:
        fprintf(stderr, "Label is out of context. It must be followed by a command or a directive.\n");

        break;

    case DIRECTIVE_NO_PARAMS:
        fprintf(stderr, "Directive has no parameters.\n");

        break;

    case DIRECTIVE_INVALID_NUM_PARAMS:
        fprintf(stderr, "Directive gets an illigal number of arguments.\n");

        break;

    case DATA_COMMAS_IN_A_ROW:
        fprintf(stderr, "Incorrect usage of commas in a .data directive.\n");

        break;

    case DATA_EXPECTED_NUM:
        fprintf(stderr, "Usage of a non-numeric arguments in a .data directive. Expected integer parameters.\n");

        break;

    case DATA_EXPECTED_COMMA_AFTER_NUM:
        fprintf(stderr, "A missing comma in a .data directive, separating integers.\n");

        break;

    case DATA_UNEXPECTED_COMMA:
        fprintf(stderr, ".data got an unexpected comma at the end of the directive line.\n");

        break;

    case STRING_TOO_MANY_OPERANDS:
        fprintf(stderr, ".string gets more then a single string.\n");

        break;

    case STRING_OPERAND_NOT_VALID:
        fprintf(stderr, ".string operand is invalid.\n");

        break;

    case EXTERN_NO_LABEL:
        fprintf(stderr, "Usage of a .extern directive without context. It must be followed by a label.\n");

        break;

    case EXTERN_INVALID_LABEL:
        fprintf(stderr, ".extern directive received an invalid label.\n");

        break;

    case EXTERN_TOO_MANY_OPERANDS:
        fprintf(stderr, ".extern gets too many lable operands.\n");

        break;

    case COMMAND_NOT_FOUND:
        fprintf(stderr, "use of an invalid command or directive.\n");

        break;

    case COMMAND_UNEXPECTED_CHAR:
        fprintf(stderr, "Invalid syntax of an assembly command.\n");

        break;

    case COMMAND_TOO_MANY_OPERANDS:
        fprintf(stderr, "Command gets to many operands. Maximal number of operands - 2.\n");

        break;

    case COMMAND_INVALID_METHOD:
        fprintf(stderr, "Operand is with unknown type. Addressing method error.\n");

        break;

    case COMMAND_INVALID_NUMBER_OF_OPERANDS:
        fprintf(stderr, "Number of operands does not match command requirements.\n");

        break;

    case COMMAND_INVALID_OPERANDS_METHODS:
        fprintf(stderr, "Operands' addressing methods do not match command requirements. Use of invalid operand for a certain command.\n");

        break;

    case ENTRY_LABEL_DOES_NOT_EXIST:
        fprintf(stderr, ".entry directive is not followed by an existing label.\n");

        break;

    case ENTRY_CANT_BE_EXTERN:
        fprintf(stderr, ".entry definition is of a label that was defined as external.\n");

        break;

    case COMMAND_LABEL_DOES_NOT_EXIST:
        fprintf(stderr, " Use of an unknown label. Lable does not exist.\n");

        break;

    case CANNOT_OPEN_FILE:
        fprintf(stderr, "There was an error while accesing file.\n");
    }
}

/* This function copies the next token of a list (comma separated e.x. 1, "abc", 4) to a dest array.
   Returns a pointer to the first character after the token */
char *nexternals_list_token(char *dest, char *line)
{
    char *temp = dest;

    if (end_of_line(line)) /* If the given line is empty, copy an empty token */
    {
        dest[0] = '\0';
        return NULL;
    }

    if (isspace(*line)) /* If there are spaces in the beginning of the token, skip them */
        line = skip_spaces(line);

    if (*line == ',') /* A comma deserves a separate, single-character token */
    {
        strcpy(dest, ",");
        return ++line;
    }

    /* Manually copying token until a ',', whitespace or end of line */
    while (!end_of_line(line) && *line != ',' && !isspace(*line))
    {
        *temp = *line;
        temp++;
        line++;
    }
    *temp = '\0';

    return line;
}

/* This function copies supposedly next string into dest array and returning a pointer to the
 * first character after it
 */
char *next_token_string(char *dest, char *line)
{
    char temp[MAX_LINE_LENGTH];
    line = nexternals_list_token(dest, line);
    if (*dest != '"')
        return line;
    while (!end_of_line(line) && dest[strlen(dest) - 1] != '"')
    {
        line = nexternals_list_token(temp, line);
        if (line)
            strcat(dest, temp);
    }
    return line;
}

/* Checking for the end of line/given token in the character that char* points to */
int end_of_line(char *line)
{
    return line == NULL || *line == '\0' || *line == '\n';
}

/* This function returns a pointer to the start of next token in the line */
char *next_token(char *seq)
{
    if (seq == NULL)
        return NULL;
    while (!isspace(*seq) && !end_of_line(seq))
        seq++;              /* Skip rest of characters in the current token (until a space) */
    seq = skip_spaces(seq); /* Skip spaces */
    if (end_of_line(seq))
        return NULL;
    return seq;
}

/* This function copies the next token (until a space or end of line) to a destination string */
void copy_token(char *dest, char *line)
{
    int i = 0;
    if (dest == NULL || line == NULL)
        return;

    while (i < MAX_LINE_LENGTH && !isspace(line[i]) && line[i] != '\0') /* Copying token until its end to *dest */
    {
        dest[i] = line[i];
        i++;
    }
    dest[i] = '\0';
}

/* This function finds an index of a string in an array of strings */
int find_index(char *token, const char *arr[], int n)
{
    int i;
    for (i = 0; i < n; i++)
        if (strcmp(token, arr[i]) == 0)
            return i;
    return NOT_FOUND;
}

/* Check if a token matches a register name */
boolean is_register(char *token)
{
    /* A register must have 2 characters, the first is 'r' and the second is a number between 0-7 */
    return (strlen(token) == REGISTER_LENGTH && token[0] == '@' && token[1] == 'r' &&
            token[2] >= '0' &&
            token[2] <= '7');
}

/* Check if a token matches a directive name */
int find_directive(char *token)
{
    if (token == NULL || *token != '.')
        return NOT_FOUND;
    return find_index(token, directives, DIRECTIVES);
}

/* Check if a token matches a command name */
int find_command(char *token)
{
    int token_len = strlen(token);
    if (token_len > MAX_COMMAND_LENGTH || token_len < MIN_COMMAND_LENGTH)
        return NOT_FOUND;
    return find_index(token, commands, COMMANDS);
}

/* This function skips spaces of a string and returns a pointer to the first non-blank character */
char *skip_spaces(char *ch)
{
    if (ch == NULL)
        return NULL;
    while (isspace(*ch)) /* Continue the loop if the character is a whitespace */
        ch++;
    return ch;
}

/* Function that skips a line if it's blank/a comment */
int skip(char *line)
{
    line = skip_spaces(line);
    return *line == ';' || *line == '\0' || *line == '\n';
}